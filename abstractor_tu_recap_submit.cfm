...
<CFQUERY datasource="#request.dsn#" NAME="read_type">
			...
			SELECT type
			FROM TRANSUNIONSVCS
			where order_num = '#rec_id#' and type = 'RECAP'
		</CFQUERY>
...
<cfquery  datasource="#request.dsn#">
			INSERT INTO
				TRANSUNIONSVCS (order_num, type, seq_num, data1, data2, data3, data4, data5, data6, data7, data8, data9, data10, data11)
				VALUES ('#rec_id#', 'RECAP', '#seq_num#', '#select1#', '#select2#', '#select3#', '#select4#', '#select5#', '#select6#', '#select7#', '#select8#', '#select9#', '#select10#', '#select11#')
