...
<!--Different Views By Date Age --->
		...
		<CFQUERY datasource="#request.dsn#" NAME="read_report13">
			SELECT *
			FROM Title a, Pay_Off_Lender_1_Title f
			...
			and a.title_id not in (18315,18960, 18982, 19065, 19117, 19142, 16085,16101,16634,16650,16733,16916,17033,17037,17053,17121,17166,17205,17227,17229,17423,17425,17607,17653,17782,18114,18208,18425,18565,18570,18731,18904,19399,19400,19324,19318,19316,19282,19447,17265,18264,18528,16966,17601,17661,17687,17977,19872, 19894, 19881,19867,19908,19881) 
		</CFQUERY>  
...
<cfif #events# neq "Final Hud Not Complete One Hour"> 
						<tr>
...
<cfif #events# eq "Final Hud Not Complete One Hour" > 
							<tr>
...
<CFQUERY datasource="#request.dsn#">		
	...
	<cfif #events# CONTAINS "Outstanding pre-processed files" or #events# eq ""> 
		<CFQUERY datasource="#request.dsn#" NAME="read_report18">
			SELECT     dbo.Title.Title_ID, dbo.Companies.company AS company, dbo.Title.check_date, dbo.Title.bfirstname1, dbo.Title.blastname1, dbo.Title.appraisal_status, 
                      dbo.Title_Closing_Order_Request.p_name, dbo.First_Admin.fname, dbo.First_Admin.lname, dbo.Title_Closing_Order_Request.dateandtime, 
                      dbo.Title_Closing_Order_Request.c_currentdate, dbo.Title_Closing_Order_Request.c_dateandtime
			FROM         dbo.Title LEFT OUTER JOIN
                      dbo.Title_Closing_Order_Request ON dbo.Title.Title_ID = dbo.Title_Closing_Order_Request.Title_ID LEFT OUTER JOIN
                      dbo.First_Admin ON dbo.Title.p_name = dbo.First_Admin.username LEFT OUTER JOIN
                      dbo.Companies ON dbo.Title.comp_id = dbo.Companies.ID
			WHERE     (dbo.Title.cancelled IS NULL) AND (dbo.Title_Closing_Order_Request.p_name IS NOT NULL) AND (DATEDIFF(hour, 
                      dbo.Title_Closing_Order_Request.dateandtime, GETDATE()) > 1) AND (dbo.Title_Closing_Order_Request.P_complete_date IS NULL)
		<cfif #order# NEQ "">
				AND Companies.comp_id = #order#
			</cfif>
		 </CFQUERY> 
...
<CFSET r_count = #read_report.recordcount#  + #read_report1.recordcount# + #read_report2.recordcount# + #read_report3.recordcount# + #read_report4.recordcount# + #read_report5.recordcount# + #read_report6.recordcount# + #read_report7.recordcount# + #read_report17.recordcount# + #read_report18.recordcount#+ #read_report8.recordcount# + #read_report9.recordcount# + #read_report11.recordcount# + #read_report12.recordcount# + #read_report13.recordcount# + #read_report14.recordcount# + #read_report10.recordcount#>
	...
	<CFQUERY datasource="#request.dsn#" NAME="read_comp">
		...
		WHERE ID = '#comp_id#'
					...
					<tr>
						...
						<td bgcolor="#a_bgcolor#" align=left valign=top>
								<FONT FACE=verdana SIZE=1 color="Blue">#read_report17.polender1#
							</td>
							<td bgcolor="#a_bgcolor#" align=left valign=top>
		...
		WHERE ID = '#comp_id#'
					...
					<tr>
						...
						<td bgcolor="#a_bgcolor#" align=left valign=top>
							...
							<td bgcolor="#a_bgcolor#"  align=right valign=top>
							<FONT FACE=verdana SIZE=1 color="blue">
<cfset totalminutes = "#DATEDIFF("n", c_dateandtime, c_currentdate)#">
<cfset hours = "#DATEDIFF("h", c_dateandtime, c_currentdate)#">
<cfset days = "#DATEDIFF("d", c_dateandtime, c_currentdate)#">
