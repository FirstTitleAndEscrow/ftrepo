...
<CFSET c_date = #DateFormat(Now(), "mm/dd/yyyy")#>
		...
		<CFQUERY DATASOURCE="#request.dsn#" >
			...
			INSERT INTO Vendors_STACIE (TSS_ID,COMPANY,USERNAME,PASSWORD,FNAME,LNAME,PHONE,FAX,CELL,PAGER,EMAIL,ADDR1,ADDR2,A_NOTE_1,CITY,STATE,ZIP_CODE,SERVICE_TYPE,A_COMMENTS,PREF_FOR_SEND,INSERT_TIME, insert_date,sb)
		...
		<cfif listFindNoCase(form.service_type,"Abstract") OR listFindNoCase(form.service_type,"TaxPrep") OR listFindNoCase(form.service_type,"ICL") OR listFindNoCase(form.service_type,"Payoff") OR listFindNoCase(form.service_type,"DeedPrep") OR listFindNoCase(form.service_type,"Typing")>
			...
			<cfif not check.recordcount>
				<cfloop list="#form.service_type#" index="service">
					...
					<cfif Len(vtype)>
						<CFQUERY DATASOURCE="#request.dsn#" >
							INSERT INTO Abstractors (type, tss_id, company, username, password, fname, lname, phone, cell, pager, fax, addr1, addr2, city, state, zip_code, email, pref_for_send,a_comments, insert_time, insert_date,sb)
							VALUES(#vtype#, '#tss_id#', '#company#', '#username#', '#password#', '#fname#',  '#lname#', '#phone#', '#cell#', '#pager#', '#fax#', '#addr1#', '#addr2#', '#city#', '#state#', '#zip_code#', '#email#',  '#pref_for_send#', '#a_comments#', '#c_time#', '#c_date#',<cfif session.ft_user_id EQ 462 OR session.ft_user_id EQ 458>
				1
			<cfelse>
				0
			</cfif>)
		...
		<cfif listFindNoCase(form.service_type,"Appraisal")>
			...
			<cfif not check_abs.recordcount>
				<CFQUERY DATASOURCE="#request.dsn#" >
					INSERT INTO Appraisers (tss_id, company, fname, lname, phone, fax, addr1, addr2, city, state, zip_code, email, pref_for_send, insert_time, insert_date, a_comments,sb)
		...
		<cfif listFindNoCase(form.service_type,"Closer")>
			...
			<cfif not check_closer.recordcount>
				<CFQUERY DATASOURCE="#request.dsn#" >
					INSERT INTO Closer (tss_id, company, username, password, fname, lname, phone, cell, pager, fax, addr1, addr2, city, state, zip_code, email, pref_for_send, a_comments, insert_time, insert_date,sb)
...
<cflocation url="https://#cgi.server_name#/admin_area/vendor_modify2.cfm?vendor_id=#Read_ID.vendor_id#&new=1">
