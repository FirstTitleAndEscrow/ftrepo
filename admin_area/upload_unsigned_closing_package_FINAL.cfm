...
<CFSET order_date_adj = '#DateFormat(Now(), "mm/dd/yyyy")# #TimeFormat(Now(), "HH:mm:ss")#'>
			...
			unsigned_closing_package = '#new_file_name#'
		...
		<CFQUERY DATASOURCE="#request.dsn#">
				...
				VALUES('#read_user.FName#', '#read_user.LName#', #URL.rec_id#, 'Unsigned Closing Package uploaded and ready for viewing', '#DateFormat(Now(), "m/d/yyyy")#', '#TimeFormat(Now(), "HH:mm:ss")#', 'T', '1', '1')
		</CFQUERY>
<CFQUERY DATASOURCE="#request.dsn#" NAME="read_user">
			select * from first_admin
			WHERE id = #url.uID#
...
<cfset documentInfo.documentDesc = "Closing Package">
<cfset documentInfo.documentType = "">
<cfset documentInfo.documentPath = "#ReplaceNoCase(fileSys.FindFilePath('#new_file_name#', 'file'), '#new_file_name#', '', 'ALL')#">

<cfset objChase.processEvent(eventCode=761, productCode=29, title_id=#read_title.title_id#,documentInfo=documentInfo, comment=form.comment)>
</cfif>
<!--- RealEC --->
...
SERVER="127.0.0.1"
Subject="First Title & Escrow - Unsigned Closing Package uploaded for #read_title.title_id#-#read_title.bfirstname1# #read_title.blastname1#<cfif #read_title.loan_number# neq "">, Loan No. - #read_title.loan_number#</cfif>"
TIMEOUT="600"
>
Unsigned Closing Package has been uploaded to our website for order #read_title.title_id#-#read_title.bfirstname1# #read_title.blastname1#
...
<CFQUERY DATASOURCE="#request.dsn#" NAME="get_file">
			select unsigned_closing_package from doc_closer_title
			WHERE title_id = #rec_ID#
</cfquery>
<CFMAIL
TO="#email#"
FROM="#team_email#"
Subject="Unsigned Closing Package file for order #rec_id#<cfif #read_title.loan_number# neq "">, Loan No. - #read_title.loan_number#</cfif>"
type="HTML"
TIMEOUT="600"
>

To view/print Unsigned Closing Package for order #rec_id# click <A HREF="https://#cgi.server_name#/admin_area/uploads/#get_file.unsigned_closing_package#" target="new">#get_file.unsigned_closing_package#</a><br><br>
Thank you<br>
First Title staff
...
<table width=455 cellpadding=1 cellspacing=1 border=0>
		...
		<B>1st Title & Escrow, Inc. - Unsigned Closing Package File Upload Complete for order <cfoutput>#rec_id#</cfoutput></B>
		<p>
		</td>
		</tr>
