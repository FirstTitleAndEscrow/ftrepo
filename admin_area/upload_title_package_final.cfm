...
<cfset new_file_name = #fileSys.MoveFromUploads('#FILE.ServerFile#')#>
		...
		<CFQUERY datasource="#request.dsn#" NAME="insert_filename">
		    update title set title_package_upload = '#new_file_name#',
			title_package_upload_date = '#order_date_adj#'
		...
		<CFQUERY datasource="#request.dsn#">		
			...
			VALUES('#read_user.FName#', '#read_user.LName#', #URL.rec_id#, 'Title Package uploaded and ready for viewing', '#DateFormat(Now(), "m/d/yyyy")#', '#TimeFormat(Now(), "HH:mm:ss")#', 'T', '1', '1')		
		</CFQUERY>
...
<table width=455 cellpadding=1 cellspacing=1 border=0>
		...
		<B>1st Title & Escrow, Inc. - Title Package File Upload Complete for order <cfoutput>#rec_id#</cfoutput></B>
		<p>
		</td>
		</tr>
