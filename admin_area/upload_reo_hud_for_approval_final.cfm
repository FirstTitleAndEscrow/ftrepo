...
<cfset new_file_name = #fileSys.MoveFromUploads('#FILE.ServerFile#')#>
		...
		<CFQUERY datasource="#request.dsn#" NAME="insert_filename">
		    update title set reo_hud_for_approval = '#new_file_name#',
			reo_hud_for_approval_date = '#order_date_adj#'
		...
		<CFQUERY datasource="#request.dsn#">		
			...
			VALUES('#read_user.FName#', '#read_user.LName#', #URL.rec_id#, 'REO HUD for Approval uploaded and ready for viewing', '#DateFormat(Now(), "m/d/yyyy")#', '#TimeFormat(Now(), "HH:mm:ss")#', 'T', '1', '1')		
		</CFQUERY>
...
<table width=455 cellpadding=1 cellspacing=1 border=0>
		...
		<B>1st Title & Escrow, Inc. - REO HUD for Approval File Upload Complete for order <cfoutput>#rec_id#</cfoutput></B>
		<p>
		</td>
		</tr>
