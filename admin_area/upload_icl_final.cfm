...
<cfset new_file_name = #fileSys.MoveFromUploads('#FILE.ServerFile#')#>
			...
			update title set ICL_upload = '#new_file_name#'
			WHERE title_id = #rec_ID#
			</cfquery> 
            <CFQUERY datasource="#request.dsn#" NAME="insert_filename">
			update doc_abstract_title set abstractor_icl_upload = '#new_file_name#'
			WHERE title_id = #rec_ID#
			</cfquery>
<CFQUERY datasource="#request.dsn#" NAME="update_ICL_Assigned">
update Vendor_Tracker_New 
set completed = 1,
return_date = '#DateFormat(Now(), "mm/dd/yyyy")#'
where order_id = #rec_ID# and Vendor_id = #session.logged_in_vendor_id#
</CFQUERY>
