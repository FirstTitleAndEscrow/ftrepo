...
<CFSET order_date_adj = '#DateFormat(Now(), "mm/dd/yyyy")# #TimeFormat(Now(), "HH:mm:ss")#'>
			...
			disbursement_sheet = '#new_file_name#'
		...
		<CFQUERY DATASOURCE="#request.dsn#">
				...
				VALUES('#read_user.FName#', '#read_user.LName#', #URL.rec_id#, 'Disbursement Sheet uploaded and ready for viewing', '#DateFormat(Now(), "m/d/yyyy")#', '#TimeFormat(Now(), "HH:mm:ss")#', 'T', '1', '1')
		</CFQUERY>
<CFQUERY DATASOURCE="#request.dsn#" NAME="read_user">
			select * from first_admin
			WHERE id = #url.uID#
...
<cfset documentInfo.documentType = "DISBURSELDGR">
<cfset documentInfo.documentDesc = "Disbursement Sheet">
<cfset documentInfo.documentPath = "#ReplaceNoCase(fileSys.FindFilePath('#new_file_name#', 'file'), '#new_file_name#', '', 'ALL')#">

<cfset objChase.processEvent(eventCode=734, productCode=29, title_id=#read_title.title_id#,documentInfo=documentInfo)>
</cfif>
<!--- RealEC --->
...
SERVER="127.0.0.1"
Subject="First Title & Escrow - Disbursement Sheet uploaded for #read_title.title_id#-#read_title.bfirstname1# #read_title.blastname1#<cfif #read_title.loan_number# neq "">, Loan No. - #read_title.loan_number#</cfif>"
TIMEOUT="600"
>
Disbursement Sheet has been uploaded to our website for order #read_title.title_id#-#read_title.bfirstname1# #read_title.blastname1#
...
<CFQUERY DATASOURCE="#request.dsn#" NAME="get_file">
			select disbursement_sheet from doc_closer_title
			WHERE title_id = #rec_ID#
</cfquery>
<CFMAIL
TO="#email#"
FROM="#team_email#"
Subject="Disbursement Sheet file for order #rec_id#<cfif #read_title.loan_number# neq "">, Loan No. - #read_title.loan_number#</cfif>"
type="HTML"
TIMEOUT="600"
>

To view/print Disbursement Sheet for order #rec_id# click <A HREF="https://#cgi.server_name#/admin_area/uploads/#get_file.disbursement_sheet#" target="new">#get_file.disbursement Sheet#</a><br><br>
Thank you<br>
First Title staff
...
<table width=455 cellpadding=1 cellspacing=1 border=0>
		...
		<B>1st Title & Escrow, Inc. - Disbursement Sheet File Upload Complete for order <cfoutput>#rec_id#</cfoutput></B>
		<p>
		</td>
		</tr>
