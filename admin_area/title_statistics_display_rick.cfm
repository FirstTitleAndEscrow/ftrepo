...
<CFQUERY DATASOURCE="#dynamic_datasource#" NAME="read_user">
	SELECT  *
	FROM	First_Admin
	where id = #url.uid#
</CFQUERY>
...
<cfif #a_trigger# eq "1">
	<CFIF #state# eq "none" and #company_id# eq "none">
		<CFQUERY DATASOURCE="#dynamic_datasource#" NAME="search1">
			SELECT *
...
<CFQUERY DATASOURCE="#dynamic_datasource#" NAME="search2">
			SELECT *
...
<!--- ======================================================================== --->
	...
	<CFLOOP QUERY="read_report">
		<CFOUTPUT>
			<CFQUERY DATASOURCE="#dynamic_datasource#" NAME="read_time">
				SELECT *
					...
					<tr>
                       ...
                       <td bgcolor=#a_bgcolor# width=110 align=center valign=top>
						...
						<td bgcolor=#a_bgcolor# width=20 align=center valign=top>
							<FONT FACE=verdana SIZE=1 color="black">
							<a href="https://#cgi.server_name#/admin_area/title_comments_nav.cfm?uid=277&al=0&rec_id=#title_id#&user_id=277&title_id=#title_id#&comments=&appraisal_status=&a_trigger=0" target="prop_hud_pg1" onClick="window.open('','prop_hud_pg1','width=620,height=500,status=0,resizable=1,scrollbars=1')">add/view</a>
						</td>
...
<CFLOOP FROM="1" TO="#ArrayLen(cty_name_1)#" INDEX="TTT">
	...
	<CFQUERY DATASOURCE="#dynamic_datasource#" NAME="read_report">
			SELXECT *
			FROM Title a, doc_title_insurance_title d, tax_cert_title e, doc_abstract_title f, pay_off_lender_1_title g
			WHERE a.title_id = d.title_id and a.title_id = e.title_id and a.title_id = f.title_id and a.title_id = g.title_id
		...
		<cfif (#confirm_mon# NEQ "Month" and #comfirm_day# NEQ "Day" and #confirm_year# NEQ "Year") and (#confirm_mon_to# NEQ "Month" and #comfirm_day_to# NEQ "Day" and #confirm_year_to# NEQ "Year")>
			...
			<cfelseif (#event18# NEQ "NONE") and (#event18# is "Vendor note added")>
...
<cfset comp_id_count = comp_id_count + 1>
		...
		<cfif (#event18# NEQ "NONE") and (#event18# is "Vendor note added")>
		...
		<cfif (#event16# NEQ "NONE") and (#event16# is "Tax Data Not Completed")>
	 	...
	 	<cfif (#event7# NEQ "NONE") and (#event7# is "Tax Data Completed")>
			AND a.title_id = e.title_id AND e.a_date_started is not null
		</cfif>
...
<CFLOOP FROM="1" TO="#ArrayLen(cty_name_1)#" INDEX="TTT">
	...
	<CFQUERY DATASOURCE="#dynamic_datasource#" NAME="read_report">
			...
			SELECT *
			FROM Title a, doc_title_insurance_title d, tax_cert_title e, doc_abstract_title f, pay_off_lender_1_title g
			WHERE a.title_id = d.title_id and a.title_id = e.title_id and a.title_id = f.title_id and a.title_id = g.title_id 	<cfif #comments# neq "none">
			<cfif #comments# is "Cancelled per client">
		and a.cancelled = 1
		<cfelse>
		and a.cancelled is null
...
<td bgcolor=#a_bgcolor# width=201 align=center valign=top>
	...
	<CFQUERY DATASOURCE="#dynamic_datasource#" NAME="read_report">
			...
			WHERE a.prop_id = d.prop_id and a.prop_id = e.prop_id and a.prop_id = f.prop_id
...
<cfset comp_id_count = comp_id_count + 1>
	  ...
	  <cfif (#event10# NEQ "NONE") and (#event10# is "Abstracts Not Assigned")>
		...
		AND a.prop_id = d.prop_id AND d.s_status is not null
			...
			<cfif (#event16# NEQ "NONE") and (#event16# is "Tax Data Not Completed")>
		and (a.prop_id = e.prop_id AND e.a_date_started is null)
		</cfif>


			<cfif (#event7# NEQ "NONE") and (#event7# is "Tax Data Completed")>
		AND a.prop_id = e.prop_id AND e.a_date_started is not null
		</cfif>
...
<cfset comp_id_count = comp_id_count + 1>
	...
	<cfif (#instructions_received# NEQ "NONE")>
	 ...
	 <cfif (#event7# NEQ "NONE") and (#event7# is "Closer Assigned")>
		...
		AND a.title_id = b.title_id AND b.pre_funding is null AND a.appraisal_status = 'Loan Closed'
		</cfif>
...
<!--- ======================================================================== --->
	...
	<CFQUERY DATASOURCE="#dynamic_datasource#" NAME="read_closer_id">
			...
			WHERE id = '#comp_id#'
					...
					<tr>
                       ...
                       <td bgcolor=#a_bgcolor# width=58 align=center valign=top>
							<FONT FACE=verdana SIZE=1 color="black">
							<cfif #yearmonth# neq "">
							#DateFormat(read_report.yearmonth, "m/d/yyyy")#</cfif>
						</td>
						<td bgcolor=#a_bgcolor# width=50 align=center valign=top>
							<FONT FACE=verdana SIZE=1 color="black">
							C-#title_id#
		...
		<td bgcolor=#a_bgcolor# width=30 align=center valign=top>
						...
						<td bgcolor=#a_bgcolor# width=30 align=center valign=top><FONT FACE=verdana SIZE=1 color="black">#prop_use_type#</font>
						</td>
...
<cfset comp_id_count = comp_id_count + 1>
 	...
 	<CFELSE>
		...
		<CFQUERY DATASOURCE="#dynamic_datasource#" NAME="read_prop">
...
<cfset comp_id_count = comp_id_count + 1>
 	...
 	<CFELSE>
		...
		<CFQUERY DATASOURCE="#dynamic_datasource#" NAME="read_closing">

			SELECT *
			FROM Title a, title_closing_order_request f
			WHERE a.title_id = f.title_id AND f.a_Req_closing_month IS NOT NULL and a.cancelled is null
...
<cfset comp_id_count = comp_id_count + 1>
 	...
 	<CFELSE>
		...
		<CFQUERY DATASOURCE="#dynamic_datasource#" NAME="read_app">
...
<cfset comp_id_count = comp_id_count + 1>
	...
	<CFQUERY DATASOURCE="#dynamic_datasource#" NAME="read_anc">
...
<cfset comp_id_count = comp_id_count + 1>
	...
	<CFQUERY DATASOURCE="#dynamic_datasource#" NAME="read_avm">
...
<cfset comp_id_count = comp_id_count + 1>
	...
	<CFQUERY DATASOURCE="#dynamic_datasource#" NAME="read_cr">
...
<cfset comp_id_count = comp_id_count + 1>
	...
	<CFQUERY DATASOURCE="#dynamic_datasource#" NAME="read_sn">
