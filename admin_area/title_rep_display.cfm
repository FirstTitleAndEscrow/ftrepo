...
<cfif (form.confirm_mon eq 9 or form.confirm_mon eq 4 or form.confirm_mon eq 6 or form.confirm_mon eq 11) and form.comfirm_day GT 30>
<cfset  form.comfirm_day = 30>
</cfif>



<cfif (form.confirm_mon_to eq 9 or form.confirm_mon_to eq 4 or form.confirm_mon_to eq 6 or form.confirm_mon_to eq 11) and form.comfirm_day_to GT 30>
<cfset  form.comfirm_day_to = 30>
</cfif>
...
<cfoutput>
<form name="excel_form" action="title_rep_display.cfm?uid=#URL.uid#&al=#URL.al#" method="post">
...
<table width=820 cellpadding=1 cellspacing=1 border=0>
    ...
    <CFQUERY datasource="#request.dsn#" NAME="read">
	 ...
	 <CFQUERY datasource="#request.dsn#" NAME="read_title_updates">
	    SELECT title_update
		from doc_abstract_title
		WHERE title_id = #title_id#	and title_update = 'True'
	</cfquery>
    <tr  >
        ...
        <td bgcolor=<cfif #SearchType# EQ "Purchase"> 00ffff <cfelse>#a_bgcolor# </cfif> width=50 align=center valign=top><FONT FACE=verdana SIZE=1 color="black">
	    <cfif #read_title_updates.recordcount#> Yes <cfelse>&nbsp;</cfif>
        </td>
...
and p.user_id = '#form.user_id#'
		...
		<cfif (IsDefined("session.ft_user_id")) and (session.mi_employee eq 1)>
	and p.order_date > '7/15/2008'
		<cfelseif (IsDefined("session.ft_user_id")) and (session.va_employee eq 1 or session.tx_employee eq 1 or session.ft_fairfax_employee eq 1 or session.mi_flushing_employee eq 1 or session.mo_employee eq 1)>
	and p.order_date > '10/01/2010'
	</cfif>
	<cfif #sortorder# NEQ "NONE" and #sortorder# eq 'title_id'>
	<cfset sortorder =  'prop_id'>
	</cfif>
...
and user_id = '#form.user_id#'
		...
		<cfif (IsDefined("session.ft_user_id")) and (session.mi_employee eq 1)>
	and check_date > '7/15/2008'
		<cfelseif (IsDefined("session.ft_user_id")) and (session.va_employee eq 1 or session.tx_employee eq 1 or session.ft_fairfax_employee eq 1 or session.mi_flushing_employee eq 1 or session.mo_employee eq 1)>
	and check_date > '10/01/2010'
	</cfif>
	<cfif #form.sortorder# NEQ "NONE">
		ORDER BY #form.sortorder#
	    <CFELSE>
...
and user_id = '#form.user_id#'
		...
		<cfif (IsDefined("session.ft_user_id")) and (session.mi_employee eq 1)>
	and c.order_date > '7/15/2008'
		<cfelseif (IsDefined("session.ft_user_id")) and (session.va_employee eq 1 or session.tx_employee eq 1 or session.ft_fairfax_employee eq 1 or session.mi_flushing_employee eq 1 or session.mo_employee eq 1)>
	and c.order_date > '10/01/2010'
	</cfif><cfif #form.sortorder# NEQ "NONE">
		ORDER BY #form.sortorder#
	 	<CFELSE>
		 ORDER BY  check_date asc 
		</cfif> </CFQUERY>  
