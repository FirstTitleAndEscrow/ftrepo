...
<CFQUERY datasource="#request.dsn#" name="write_note">		
				...
				VALUES('Web', 'System', #form.rec_ID#, 'REO Package Received from Vendor', '#DateFormat(Now(), "m/d/yyyy")#', '#TimeFormat(Now(), "HH:mm:ss")#', '#form.order_type#', '1', '1')		
		</CFQUERY>
...
<CFQUERY datasource="#request.dsn#" name="write_note">		
				INSERT INTO Title_Notes(FName, LName, Order_ID, N_Note, N_Date, N_Time, note_type, access, access2)
				VALUES('Web', 'System', #form.rec_ID#, 'REO Package Completed and Uploaded In-House', '#DateFormat(Now(), "m/d/yyyy")#', '#TimeFormat(Now(), "HH:mm:ss")#', '#form.order_type#', '1', '1')		
		</CFQUERY>
...
<CFQUERY datasource="#request.dsn#">		
				INSERT INTO Title_Notes(FName, LName, Order_ID, N_Note, N_Date, N_Time, note_type, access, access2)
				VALUES('#read_user.FName#', '#read_user.LName#', #URL.rec_id#, 'REO Package Retrieval assigned to Vendor: #read_vendor.company#', '#DateFormat(Now(), "m/d/yyyy")#', '#TimeFormat(Now(), "HH:mm:ss")#', '#form.order_type#', '1', '1')		
		</CFQUERY>	
...
<cfif form.reo_commit eq 1>
<cflocation url="vendor_manager.cfm?selected_reo_vendor_id=0&uid=#url.uid#&al=#url.al#&company_id=#url.company_id#&rec_id=#url.rec_id#&order_type=#url.order_type#&code=#url.code###reo_plot" addtoken="no">
<cfelse>
<cflocation url="vendor_manager.cfm?selected_reo_vendor_id=#form.vendor_id#&uid=#url.uid#&al=#url.al#&company_id=#url.company_id#&rec_id=#url.rec_id#&order_type=#url.order_type#&code=#url.code###reo_plot" addtoken="no">
</cfif>
<cfabort>
