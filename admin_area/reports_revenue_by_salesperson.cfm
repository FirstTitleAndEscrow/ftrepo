...
FROM         First_Admin
WHERE    
<cfif session.role_id eq 34 or session.ft_user_id eq 248 or session.ft_user_id eq 59 or session.ft_user_id eq 56>
...
<body>
<CFFORM NAME="frm" ACTION="./reports_revenue_by_salesperson_result.cfm?uid=#url.uid#" METHOD=POST>
<cfoutput>
<cfset bgcolor = 'efefef'>
    <table width="75%" Border="0">
        ...
        <TR>
            <td colspan="2">
                <B><Center>Sales Revenue Report</center></B>
            </td>     
        </TR>
        <CFIF #a_trigger# eq "1">
        <TR>
