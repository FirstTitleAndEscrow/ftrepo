...
<CFIF #Resched# eq 1>
	<CFQUERY datasource="#request.dsn#">
		INSERT INTO Title_Notes(FName, LName, Order_ID, N_Note, N_Date, N_Time, note_type, access, access2)
				VALUES('#trim(read_user.fname)#', '#trim(read_user.lname)#', #URL.rec_id#, 'Closing Postponed, Original Fee.','#DateFormat(Now(), "m/d/yyyy")#', '#TimeFormat(Now(), "HH:mm:ss")#', 'T', '1', '1')
	</CFQUERY>
...
<CFQUERY datasource="#request.dsn#" NAME="get_richmond">
	SELECT ID from companies where intSales in (Select ID from First_Admin where va_employee = 'True'
</CFQUERY>
<cfset richmond_ids = ValueList(get_richmond.ID, ",")>
<cfif ListContainsNoCase(richmond_ids, get_title.comp_id, ",")>
<cfset from_email = 'richmond@firsttitleservices.com'>
<CFSET send_mail = #send_mail# & ";richmond@firsttitleservices.com">
<cfelse>
<cfset from_email = 'closings@firsttitleservices.com'>
	...
	<CFMAIL
		TO="#send_mail#"
		FROM="#from_email#" Subject="First Title Order Number #get_title.title_id#"
		TIMEOUT="600"
		type="HTML"
		>
...
<CFELSEIF #Resched# eq 2>
	...
	<CFIF #Result# eq 0>
		...
		<CFMAIL
			...
			FROM="closings@firsttitleservices.com" Subject="#get_title.title_id# Problem."
			TIMEOUT="600"
			type="HTML"
		>
...
<cfif is_VA.va_employee eq 'True'>
	...
	<CFSET send_mail = #send_mail# & ";closingdepartment@union-mtg.com">
		...
		<CFMAIL
		TO="#send_mail#"
		FROM="closings@firsttitleservices.com" Subject="First Title Order Number #get_title.title_id#"
		TIMEOUT="600"
		type="HTML"
		>
	Closing Rescheduled for Order Number-#get_title.title_ID# Borrower - #get_title.bfirstname1# #get_title.blastname1#.  There will be an additional fee of $#Fee#.
	</CFMAIL>
	</CFIF>

	<CFQUERY datasource="#request.dsn#">
		INSERT INTO Title_Notes(FName, LName, Order_ID, N_Note, N_Date, N_Time, note_type, access, access2)
				VALUES('#trim(read_user.fname)#', '#trim(read_user.lname)#', #URL.rec_id#, 'Closing Rescheduled. Fee is $#Fee#', '#DateFormat(Now(), "m/d/yyyy")#', '#TimeFormat(Now(), "HH:mm:ss")#', 'T', '1', '1')
	</CFQUERY>
...
<cfif #read_title.appraisal_status# neq "Loan Closed">
	...
	<CFQUERY datasource="#request.dsn#">
		INSERT INTO Title_Notes(FName, LName, Order_ID, N_Note, N_Date, N_Time, note_type, access, access2)
				VALUES('#trim(read_user.fname)#', '#trim(read_user.lname)#', #URL.rec_id#, 'Cancelled, Fee to Closer', '#DateFormat(Now(), "m/d/yyyy")#', '#TimeFormat(Now(), "HH:mm:ss")#', 'T', '1', '1')
	</CFQUERY>
...
<cfcatch type="Any">
	...
	<CFMAIL
		TO="#send_mail#"
		FROM="#team_email#" Subject="First Title Order Number #get_title.title_id#"
		TIMEOUT="600"
		type="HTML"
		>
	Closing Cancelled for First Title Order Number -#get_title.title_ID# Borrower - #get_title.bfirstname1# #get_title.blastname1#. Fee to Closer.<br>
<br>
