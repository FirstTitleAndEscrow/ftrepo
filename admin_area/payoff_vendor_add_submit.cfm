<CFQUERY datasource="#request.dsn#" NAME="check_payoff">
	SELECT *
	FROM Abstractors
	WHERE Company = '#form.company#' AND lname = '#form.lname#' AND fname = '#form.fname#' and type = 6
</CFQUERY>
<cfif check_payoff.recordcount>
<script language="javascript">
alert("It appears that this particular Payoff Vendor is already listed in the system");
history.go(-1);
</script>
<cfabort>
</cfif>		
...
<CFSET zip_id_2 = #ListToArray(zip_id, ",")#>
	...
	<CFSET cty_name_1 = ArrayNew(1)>
		...
		<CFQUERY datasource="#request.dsn#" >
			...
			VALUES(6, '#tss_id#', '#company#', '#username#', '#password#', '#fname#',  '#lname#', '#phone#', '#cell#', '#pager#', '#fax#', '#addr1#', '#addr2#', '#city#', '#state#', '#zip_code#', '#email#', '#a_cost#',  '#pref_for_send#',  '#a_note_1#', '#a_comments#', '#c_time#', '#c_date#', '#oe#', '#deedonly#', '#oe1#', '#oe2#', '#mort#', '#rec#', '#judg#', '#rec_update#', '#refphone1#', '#refphone2#', '#refCo1#', '#refCo2#', '#city2#', '#state2#', '#zip_code2#', '#SSN#', '#TAXID#', '#homephone#', '#otherphone#', '#eocompany#', '#eoamount#', '#eoexpiredate#')
...
<CFOUTPUT>
	<tr>
		<td width=498 align=center valign=top bgcolor=white>
			<font size=2 color=green face=arial><B>The Payoff Vendor was added</B>
		</td>
	</tr>
</CFOUTPUT>	
...
FROM="webmaster@firsttitleservices.com" Subject="Payoff Vendor added to First Title Database"
TIMEOUT="600"
type="HTML"
>
...
<tr>
	...
	<tr>		
		<td width=468 colspan=2 align=center valign=top bgcolor=e6e6e6>	
			<font size=2 color=gray face=arial>
			Preferences for recieving Payoff Vendor Notice
