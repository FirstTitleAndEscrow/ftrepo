...
<cfif session.ft_user_id eq 59 or session.ft_user_id eq 56 or session.ft_user_id eq 248 or session.role_id eq 34 or session.ft_user_id eq 547>
...
WHERE     (status = 1 or status = 'True') and ((role_id = 24 OR (role_id = 34 AND id <> 450)) <!--- AND (va_employee IS NULL OR
                      va_employee = 0 or  
                      va_employee = 'False') AND (mi_employee IS NULL OR
                      mi_employee = 0 or  
                      mi_employee = 'False') AND (mo_employee IS NULL OR
                      mo_employee = 0 or  
                      mo_employee = 'False') AND (mi_flushing_employee IS NULL OR
                      mi_flushing_employee = 0 or mi_flushing_employee = 'False') AND (tx_employee IS NULL OR
                      tx_employee = 0 or  
                      tx_employee = 'False') AND (ft_fairfax_employee IS NULL OR
                      ft_fairfax_employee = 0 or  
                      ft_fairfax_employee = 'False') ---> AND (ID NOT IN (327, 264, 450)) OR
                      (ID = 319) OR
                      (ID = 408) OR
                      (ID = 460))
ORDER BY lname
...
<form action="reports_active_no_orders_30_days.cfm?sort_order_3=#url.sort_order_3#&UID=#url.uid#&al=1&sort_order_1=#url.sort_order_1#&sort_order_2=#url.sort_order_2#&sort_order_4=#url.sort_order_4#" method="post" name="sale_stats_form">
<cfset bgcolor = 'efefef'>
<table  class="table_border">
<tr>
<td colspan=3 align="top" valign="left"><span class="table_header"><strong>Date Range</strong></span></td>
<cfif session.ft_user_id eq 59 or session.ft_user_id eq 56 or session.ft_user_id eq 248 or session.role_id eq 34 or session.ft_user_id eq 547><td align="top" valign="left"><span class="table_header"><strong>Salesperson</strong></span></td></cfif>
</tr>
<tr>
...
<cfif session.ft_user_id neq 59 and session.ft_user_id neq 56 and session.ft_user_id neq 248 and session.ft_user_id neq 450 and session.ft_user_id neq 460 and session.ft_user_id neq 462 and get_user.fairfax_admin neq 1 and session.ft_user_id neq 547>
...
and c.id NOT IN (Select comp_id from title where order_date between #odbc_start_date# and #odbc_end_date#)
and (c.last_order_date between  #odbc_sixty_days# and #odbc_start_date#)
and c.team_id = m.ID
and c.IntSales = f.ID
