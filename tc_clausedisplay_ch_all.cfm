...
<CFSET a_down_trigger = "1">
			...
			<CFIF #a_section_code_for_this_section# EQ "B1">
				<CFQUERY datasource="#request.dsn#" NAME="read_temp_comm_12">		
					SELECT *
					FROM Title_Committment_Clauses_Selected_Title
					WHERE Title_ID = #rec_ID# AND a_section_code = '#a_section_code_for_this_section#' AND Committment_Item_Order > 1
					ORDER BY Committment_Item_Order ASC
				</CFQUERY>			
			<CFELSEIF #a_section_code_for_this_section# EQ "B2">
				<CFQUERY datasource="#request.dsn#" NAME="read_temp_comm_12">		
					SELECT *
					FROM Title_Committment_Clauses_Selected_Title
					WHERE Title_ID = #rec_ID# AND a_section_code = '#a_section_code_for_this_section#' AND Committment_Item_Order < 7
					ORDER BY Committment_Item_Order ASC
				</CFQUERY>
