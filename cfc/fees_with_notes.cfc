...
<cfif #arguments.transaction_type# eq "Purchase">
	...
	<cfset purchase_price = arguments.purchase_price>
<cfset loan_amount = arguments.loan_amount>

<cfif loan_amount gt purchase_price>
	<cfset amount_to_use = loan_amount>
<cfelse>
	<cfset amount_to_use = purchase_price>
</cfif>

<cfif arguments.property_county eq 'Montgomery'>
	<cfif (arguments.property_use_type eq '1'  or arguments.property_use_type eq 'Primary Residence') and amount_to_use GT 50000>
	<cfset amount_to_use = amount_to_use - 50000>
	</cfif>


<!--- gotta build in the same 500,00 dollar break, same as below --->
<!--- if primary residence and a purchase, then take 50,0000 off loan_amount --->

	<cfif amount_to_use lt 500000>
	<cfset city_mort_stamps = read_rec_fees.mort_stamps * amount_to_use>
	<cfelse>
	<cfset city_mort_stamps = read_rec_fees.mort_stamps * 500000>
	<cfset city_mort_stamps = city_mort_stamps + ((amount_to_use - 500000)*.01)>
	</cfif>
<cfelse>
<cfset city_mort_stamps = read_rec_fees.mort_stamps * amount_to_use>
</cfif>
...
<cfset city_mort_stamps = 0>
<!--- if is a refi and it's primary residence, if new loan amount is over 500k and old loan is over 500k, whole thing gets taxed at $10 per thousand

Iif old loan amount is under 500k and new loan is over 500k, subtract hwtaver is needed to bring the old loan up to 500K friom the new loan amount....then tax the 500k old loan at the old rate, and whatever number is left for the new loan at the new rate ($10/1000) --->
...
<cfif (arguments.property_use_type eq '1'  or arguments.property_use_type eq 'Primary Residence') AND arguments.property_county eq 'Montgomery' and arguments.loan_amount GT arguments.previous_loan_amount>
	<cfif arguments.loan_amount GTE 500000 and arguments.previous_loan_amount GTE 500000>
	<cfset city_mort_stamps=#NumberFormat(ROUND((arguments.loan_amount-arguments.previous_loan_amount)*.01),'_____.__')#>
	<cfelseif arguments.previous_loan_amount LT 500000 and arguments.loan_amount gte 500000>
	<cfset new_previous_loan_amount = (500000 - arguments.previous_loan_amount)>
	<cfset city_mort_stamps = #NumberFormat(ROUND((new_previous_loan_amount)*read_rec_fees.mort_stamps),'_____.__')#>
	<cfset new_loan_amount = (arguments.loan_amount - (new_previous_loan_amount))>
	<cfset city_mort_stamps=city_mort_stamps + #NumberFormat(ROUND((new_previous_loan_amount)*read_rec_fees.mort_stamps),'_____.__')#>
	<cfelse>
...
<CFIF arguments.property_state eq "TX" and (arguments.transaction_type eq "Refinance" or arguments.transaction_type eq "Second Mortgage")>
