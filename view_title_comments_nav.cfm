...
<CFPARAM NAME="A_Update_DB_Flag" DEFAULT="0">
	...
	<CFIF #A_Update_DB_Flag# GT "0">
		...
		<CFQUERY datasource="#request.dsn#">
				...
				VALUES('#username#', #URL.rec_id#, '#N_Note_1#', '#DateFormat(Now(), "m/d/yyyy")#', '#TimeFormat(Now(), "HH:mm:ss")#', '#url.code#', '1', '1')
			</CFQUERY>

			<CFQUERY datasource="#request.dsn#" NAME="read_info">
				SELECT *
				FROM Title_Notes
				WHERE N_date = '#DateFormat(Now(), "m/d/yyyy")#' AND N_Time = '#TimeFormat(Now(), "HH:mm:ss")#' and order_id = #url.rec_id# and note_type = '#url.code#'
			</CFQUERY>
		...
		<CFQUERY datasource="#request.dsn#" NAME="read_notes">
				SELECT *
				FROM Title_Notes
				WHERE order_id = #URL.rec_id# and N_Note = 'Clear to Close' and note_type = '#url.code#'
			</CFQUERY>
			<cfif #read_notes.recordcount# GT 0>
		<CFQUERY datasource="#request.dsn#">
				Update Title_Notes
				set status = 1
				where note_id = #read_info.note_id# and N_Note != 'Clear to Close' and note_type = '#url.code#'
			</CFQUERY>
			<CFQUERY datasource="#request.dsn#">
		...
		<cfelse>
		<cfif url.code eq 'P' or url.code eq 'T'>
		<CFQUERY datasource="#request.dsn#">
				Update Title_Notes
				set status = 1
				where note_id = #read_info.note_id# and N_Note != 'Clear to Close' and note_type = '#url.code#'
...
<cfelseif url.code eq 'FDR'>
	<CFQUERY datasource="#request.dsn#" NAME="read_email">
				SELECT *
				FROM Final_Document_Retrieval
				Where fdr_id = #rec_id#
			</CFQUERY>
<cfelseif url.code eq 'PR'>
	<CFQUERY datasource="#request.dsn#" NAME="read_email">
				SELECT *
...
<cfset send_email = #team_email#>
<cfif url.code eq 'FDR' or  url.code eq 'PR'>
<cfset send_email = send_email & "; " & "UWMDocs@firsttitleservices.com">
</cfif>
...
<cfcatch type="Any">
		...
		<hr>
			...
			<CFQUERY datasource="#request.dsn#" NAME="read_all_notes">
				SELECT *
				FROM Title_Notes
				Where Order_Id = #URL.rec_id#  and note_type = '#url.code#' and access = 1
...
<table width=590 cellpadding=1 cellspacing=1 border=0 bgcolor=gray>
		...
		<tr>
			...
			<td width=590 align=center valign=top bgcolor=green>
				...
				<b>View Comments for order <CFOUTPUT>#url.code#-#url.rec_id#</CFOUTPUT></a>
		</tr>
...
<CFFORM NAME="frm" ACTION="./view_Title_comments_nav.cfm?user_id_1=#USER_ID_1#&lo_id=#lo_id#&rec_id=#url.rec_id#&a_trigger=1&Title_Ins_Co_Is=#Title_Ins_Co_Is#&A_Update_DB_Flag=1&code=#url.code#" onsubmit="javascript:return locate(this)" ENCTYPE="multipart/form-data" METHOD=POST>
